...
**Deleting is built — step 9a, version 10.** `v2/Manage.js`. **Re-run
`bootstrap()` once before using it**: it now creates a `<Section> Archive` sheet
per section, and deleting fails clearly until they exist.

`Delete` in the table archives the row and moves its documents to `Archived`. It
removes nothing. The **Archive** button in the header shows what is in there,
with `Restore` and `Delete forever` per row.

- **Hard delete's safeguard is structural, not a scarier dialog.**
  `hardDeleteEntry()` only ever resolves a row against the **archive sheet**, so
  live data cannot be destroyed in one action from anywhere — the function
  cannot see it. Archive first, then purge, in two different places.
- **Archiving writes the copy before it removes anything.** Every ordering has a
  failure mode; this one's is a row existing in both places, which is visible
  and repairable. The alternative is losing it.
- **Restore re-files documents through `applyFileState`** rather than moving
  them back where they were, so the folder *and* the filename suffix chain are
  rebuilt from the row's own dates. Same code path as a status change, which is
  what stops restore drifting away from it.
- **Files are trashed, never purged.** 30 days in Drive's trash is the
  difference between a mistake and a loss.
- **Archive headers are generated by the same `sectionHeaders()`** the live
  sheet uses, plus `Archived` and `Archive Reason`, so the pair cannot drift.

Confirmations exist for these two actions and nowhere else — deleting is
...
| `v2/Manage.js` | Archive, restore, hard delete, and the archive listing |
